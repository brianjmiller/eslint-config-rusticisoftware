...
describe("multiline-ternary", function () {
    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "const x = a ?\n    b :\n    c;",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "multiline-ternary");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "multiline-ternary", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
