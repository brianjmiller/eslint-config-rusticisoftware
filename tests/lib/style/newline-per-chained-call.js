...
/**
 * Tests for newline-per-chained-call
...
describe("newline-per-chained-call", function () {
    const linter = new Linter();

    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "obj.method1()\n    .method2()\n    .method3();",
                messages = linter.verify(code, config),
                relevantMessages = messages.filter((msg) => msg.ruleId === "newline-per-chained-call");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "newline-per-chained-call", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
