    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "function foo() {\n    const x = 1;\n\n    return x;\n}",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "newline-before-return");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "newline-before-return", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
