    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "const x = a || b;",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "no-ternary");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "no-ternary", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
