...
describe("init-declarations", function () {
    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "let x = 1;",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "init-declarations");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "init-declarations", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
