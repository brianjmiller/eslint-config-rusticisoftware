 ...
 */
 ...
 const {Linter} = require("eslint"),
    ...
    describe("prefer-spread", function () {
    const linter = new Linter();

    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "Math.max(...arr);",
                messages = linter.verify(code, config),
                relevantMessages = messages.filter((msg) => msg.ruleId === "prefer-spread");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "prefer-spread", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
