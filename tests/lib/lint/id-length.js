    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "const myVar = 1;",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "id-length");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "id-length", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
