 ...
 */
 ...
 const {Linter} = require("eslint"),
    ...
    describe("no-warning-comments", function () {
    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "// Regular comment",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "no-warning-comments");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "no-warning-comments", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
