...
describe("no-nested-ternary", function () {
    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "const x = a ? b : c;",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "no-nested-ternary");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "no-nested-ternary", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
