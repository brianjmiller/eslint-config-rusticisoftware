 ...
 */
 ...
 const {Linter} = require("eslint"),
    ...
    describe("require-jsdoc", function () {
    const linter = new Linter();

    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "/** JSDoc */ function foo() {}",
                messages = linter.verify(code, config),
                relevantMessages = messages.filter((msg) => msg.ruleId === "jsdoc/require-jsdoc");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "jsdoc/require-jsdoc", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
