 ...
 */
 ...
 const {Linter} = require("eslint"),
    ...
    describe("prefer-template", function () {
    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "const str = `hello ${name}`;",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "prefer-template");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "prefer-template", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
    describe("rule is disabled", function () {
        it("should not report errors (rule is off)", function () {
            ...
            const testCode = "const str = \"hello \" + name;",
