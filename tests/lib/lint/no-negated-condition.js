...
/**
 * Tests for no-negated-condition
 ...
 */
 ...
 const {Linter} = require("eslint"),
    ...
    describe("no-negated-condition", function () {
    ...
    describe("valid code", function () {
        it("valid case 1", function () {
            const code = "if (x) {} else {}",
                ...
                relevantMessages = messages.filter((msg) => msg.ruleId === "no-negated-condition");

            assert.strictEqual(
                relevantMessages.length,
                0,
                `Expected no errors for "no-negated-condition", but got: ${relevantMessages.map((m) => m.message).join(", ")}`
            );
        });
    });
